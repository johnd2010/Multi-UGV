...
Integration of GPS data is a common request from users. ``jackal_robot_localization`` contains a node, ``navsat_transform_node``, that transforms GPS data into a frame that is consistent with your robot's starting pose (position and orientation) in its world frame. This greatly simplifies fusion of GPS data. This tutorial explains how to use ``navsat_transform_node``, and delves into some of the math behind it.
...
1. Run one instance of a ``jackal_robot_localization`` state estimation node that fuses only continuous data, such as odometry and IMU data. Set the ``world_frame`` parameter for this instance to the same value as the ``odom_frame`` parameter. Execute local path plans and motions in this frame.
2. Run another instance of a ``jackal_robot_localization`` state estimation node that fuses all sources of data, including the GPS. Set the ``world_frame`` parameter for this instance to the same value as the ``map_frame`` parameter. 

This is just a suggestion, however, and users are free to fuse the GPS data into a single instance of a ``jackal_robot_localization`` state estimation node. 
 ...
 * A `nav_msgs/Odometry <http://docs.ros.org/api/nav_msgs/html/msg/Odometry.html>`_ message that contains the robot's current position estimate in the frame specified by its start location (typically the output of a ``jackal_robot_localization`` state estimation node).
...
2. The datum (global frame origin) can be specified via the ``datum`` parameter. 
 ...
 The parameter order is ``latitude`` in decimal degrees, ``longitude`` in decimal degrees, ``heading`` in radians) the ``frame_id`` of your robot's world frame (i.e., the value of the ``world_frame`` parameter in a ``jackal_robot_localization`` state estimation node), and the ``frame_id`` of your robot's body frame (i.e., the value of the ``base_link_frame`` parameter in a ``jackal_robot_localization`` state estimation node). When this mode is used, the robot assumes that your robot's world frame origin is at the specified latitude and longitude and with a heading of :math:`0` (east). 

3. The datum can be set manually via the ``set_datum`` service and using the `jackal_robot_localization/SetDatum <http://docs.ros.org/api/jackal_robot_localization/html/srv/SetDatum.html>`_ service message. 
...
This should just be the output of whichever ``jackal_robot_localization`` state estimation node instance you are using to fuse GPS data.
...
.. code-block:: xml
 ...
 <launch>
   ...
   <node pkg="jackal_robot_localization" type="navsat_transform_node" name="navsat_transform_node" respawn="true">
     ...
     <remap from="/odometry/filtered" to="/your/jackal_robot_localization/output/topic" />
...
Integration with ``jackal_robot_localization`` is straightforward at this point. Simply add this block to your state estimation node launch file:
 ...
 * Set ``two_d_mode`` to *true* in your ``jackal_robot_localization`` state estimation node
